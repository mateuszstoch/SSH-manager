...
# Configuration
# -------------------------------------------------------------------------
REPO_URL="https://github.com/mateuszstoch/SSH-manager"

if [[ "$VERSION" == "latest" ]]; then
    DOWNLOAD_URL="$REPO_URL/releases/latest/download/sm-$target.$ext"
else
    DOWNLOAD_URL="$REPO_URL/releases/download/$VERSION/sm-$target.$ext"
fi

install_env=SM_INSTALL
bin_env=\$$install_env/bin

install_dir=${SM_INSTALL:-"$HOME/.sm"}
...
# Installation
# -------------------------------------------------------------------------
info_bold "Installing sm ($VERSION) for $target..."
...
if ! curl --fail --location --progress-bar --output "sm_download.$ext" "$DOWNLOAD_URL"; then
    echo
...
info "Extracting..."
if [[ "$ext" == "zip" ]]; then
    unzip -q -o -j "sm_download.$ext" -d "$bin_dir" || error "Failed to unzip archive"
else
    tar -xzf "sm_download.$ext" -C "$bin_dir" || error "Failed to extract tar archive"
fi

# Cleanup
rm -f "sm_download.$ext"
...
success "sm ($VERSION) was installed successfully to $Bold_Green$(tildify "$exe")"
